...
from diffuser.utils import at_least_ndim, Mlp
from diffuser.nn_condition import IdentityCondition, get_mask
...
class MLPSieveObsCondition(IdentityCondition):
    def __init__(
        ...
        self.mlp = Mlp(o_dim, hidden_dims, emb_dim, nn.LeakyReLU())

    def forward(self, obs: torch.Tensor, mask: torch.Tensor = None):
        mask = at_least_ndim(
            get_mask(mask, (obs.shape[0],), self.dropout, self.training, obs.device), 2
        )
