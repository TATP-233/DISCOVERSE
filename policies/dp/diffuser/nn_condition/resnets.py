...
class ResNet18ImageCondition(BaseNNCondition):
    ...
    def forward(self, condition: torch.Tensor, mask: torch.Tensor = None):
        ...
        assert (
            condition_dim == 4 or condition_dim == 5
        ), "Input condition must be 4D or 5D tensor, got shape {}".format(
            condition.shape
        )
        ...
        mask = get_mask(mask, (b,), self.dropout, self.training, condition.device)
...
class ResNet18MultiViewImageCondition(BaseNNCondition):
    ...
    def forward(self, condition: torch.Tensor, mask: torch.Tensor = None):
        ...
        condition_dim = condition.dim()
        assert (
            condition_dim == 5 or condition_dim == 6
        ), "Input condition must be 5D or 6D tensor, got shape {}".format(
            condition.shape
        )
        ...
        if condition_dim == 6:
            emb = einops.rearrange(emb, "(b n) v d -> b v n d", b=b) * at_least_ndim(
                mask, 4
            )
