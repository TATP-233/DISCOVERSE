...
class BaseDiffusionSDE(DiffusionModel):
    ...
    def clip_prediction(self, pred, xt, alpha, sigma):
        ...
        if self.predict_noise:
            if self.clip_pred:
                upper_bound = (
                    (xt - alpha * self.x_min) / sigma
                    if self.x_min is not None
                    else None
                )
                lower_bound = (
                    (xt - alpha * self.x_max) / sigma
...
class DiscreteDiffusionSDE(BaseDiffusionSDE):
    ...
    def __init__(
        ...
        if isinstance(discretization, str):
            if discretization in SUPPORTED_DISCRETIZATIONS.keys():
                self.t_diffusion = SUPPORTED_DISCRETIZATIONS[discretization](
                    diffusion_steps, epsilon
                ).to(device)
            else:
                Warning(
                    f"Discretization method {discretization} is not supported. "
                    f"Using uniform discretization instead."
                )
                self.t_diffusion = SUPPORTED_DISCRETIZATIONS["uniform"](
                    diffusion_steps, epsilon
                ).to(device)
        ...
        if isinstance(noise_schedule, str):
            if noise_schedule in SUPPORTED_NOISE_SCHEDULES.keys():
                self.alpha, self.sigma = SUPPORTED_NOISE_SCHEDULES[noise_schedule][
                    "forward"
    ...
    def add_noise(self, x0, t=None, eps=None):
        ...
        alpha, sigma = at_least_ndim(self.alpha[t], x0.dim()), at_least_ndim(
            self.sigma[t], x0.dim()
        )
    ...
    def sample(
        ...
        **kwargs,
        ...
        if isinstance(warm_start_reference, torch.Tensor):
            ...
            fwd_alpha, fwd_sigma = (
                self.alpha[diffusion_steps],
                self.sigma[diffusion_steps],
            )
        ...
        if isinstance(sample_step_schedule, str):
            if sample_step_schedule in SUPPORTED_SAMPLING_STEP_SCHEDULE.keys():
                sample_step_schedule = SUPPORTED_SAMPLING_STEP_SCHEDULE[
        ...
        stds[1:] = (
            sigmas[:-1] / sigmas[1:] * (1 - (alphas[1:] / alphas[:-1]) ** 2).sqrt()
        )
        ...
        for i in reversed(loop_steps):
            ...
            eps_theta = (
                pred
                if self.predict_noise
                else xtheta_to_epstheta(xt, alphas[i], sigmas[i], pred)
            )
            x_theta = (
                pred
...
class ContinuousDiffusionSDE(BaseDiffusionSDE):
    ...
    def add_noise(self, x0, t=None, eps=None):
        ...
        t = (
            (
                torch.rand((x0.shape[0],), device=self.device)
    ...
    def sample(
        ...
        **kwargs,
        ...
        prior = prior.to(self.device)
        if (
            isinstance(warm_start_reference, torch.Tensor)
            and warm_start_forward_level > 0.0
        ):
            warm_start_forward_level = self.epsilon + warm_start_forward_level * (
                1.0 - self.epsilon
            )
