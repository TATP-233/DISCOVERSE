...
from diffuser.utils import at_least_ndim, cosine_beta_schedule, linear_beta_schedule
...
class DDPM(DiffusionModel):
    ...
    def add_noise(self, x0, t=None, eps=None):
        t = (
            torch.randint(self.diffusion_steps, (x0.shape[0],), device=self.device)
            if t is None
            else t
        )
    ...
    def loss(self, x0, condition=None):
        # print('train')
        # print('image0', condition['image0'].shape) # Batch size, Sample sequence length, 3, H, W
        # print('image1', condition['image1'].shape)
        # print('agent_pos', condition['agent_pos'].shape) # Batch size, Sample sequence length, Pos num
    ...
    def predict_function(
        ...
        if self.predict_noise:
            if self.clip_pred:
                upper_bound = (
                    (x - bar_alpha.sqrt() * self.x_min) / (1 - bar_alpha).sqrt()
                    if self.x_min is not None
                    else None
                )
                lower_bound = (
                    (x - bar_alpha.sqrt() * self.x_max) / (1 - bar_alpha).sqrt()
    ...
    def sample(
        ...
        **kwargs,
        ...
        if sample_steps != self.diffusion_steps:
            ...
            warnings.warn(
                f"sample_steps != diffusion_steps, sample_steps will be set to diffusion_steps."
            )
        ...
        for t in range(self.diffusion_steps - 1, -1, -1):
            ...
            t_batch = torch.tensor(t, device=self.device, dtype=torch.long).repeat(
                n_samples
            )
            ...
            if self.predict_noise:
                ...
                xt = (
                    1 / alpha.sqrt() * (xt - beta / (1 - bar_alpha).sqrt() * pred_theta)
                )
            ...
            if t != 0:
                xt = xt + (
                    beta * (1 - bar_alpha_prev) / (1 - bar_alpha)
    ...
    def sample_x(
        ...
        **kwargs,
        ...
        if extra_sample_steps > 0:
            ...
            t_batch = torch.tensor(0, device=self.device, dtype=torch.long).repeat(
                n_samples
            )
            ...
            for _ in range(extra_sample_steps):
                ...
                if self.predict_noise:
                    ...
                    xt = (
                        1
                        / alpha.sqrt()
                        * (xt - beta / (1 - bar_alpha).sqrt() * pred_theta)
                    )
