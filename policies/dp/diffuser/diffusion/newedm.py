...
class ContinuousEDM(DiffusionModel):
    ...
    def add_noise(self, x0, t=None, eps=None):
        ...
        t = (
            (
                torch.randn((x0.shape[0],), device=self.device) * self.P_std
                + self.P_mean
            ).exp()
            if t is None
            else t
    ...
    def update_classifier(self, x0, condition):
        ...
        log = self.classifier.update(xt, t.log() / 4.0, condition)
    ...
    def classifier_guidance(
        self, xt, t, sigma, model, condition=None, w: float = 1.0, pred=None
    ):
        """
        Guided Sampling CG:
        ...
        with torch.set_grad_enabled(requires_grad):
            if w != 0.0 and w != 1.0:
                if pred is None or pred_uncond is None:
                    ...
                    pred_all = self.D(
                        xt.repeat(*repeat_dim), t.repeat(2), condition, model
                    )
    ...
    def sample(
        ...
        **kwargs,
        ...
        if (
            ...
            fwd_sigma = (
                self.sigma_min
                + (self.sigma_max - self.sigma_min) * warm_start_forward_level
            )
            xt = warm_start_reference + fwd_sigma * torch.randn_like(
        ...
        if self.classifier is not None:
            with torch.no_grad():
                t = (
                    torch.ones((n_samples,), dtype=torch.long, device=self.device)
                    * self.sigma_min
                )
