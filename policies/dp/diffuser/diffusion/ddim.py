...
class DDIM(DiffusionModel):
    ...
    def sample(
        ...
        t = torch.linspace(
            self.t_range[1], self.t_range[0], sample_steps + 1, device=self.device
        )
        ...
        for i in range(sample_steps):
            ...
            with torch.no_grad():
                if w_cfg != 0.0 and w_cfg != 1.0:
                    ...
                    eps_theta = (
                        w_cfg * eps_theta[:n_samples]
                        + (1.0 - w_cfg) * eps_theta[n_samples:]
                    )
                else:
            ...
            if self.classifier is not None and w_cg != 0.0:
                log_p, grad = self.classifier.gradients(
                    xt.clone(), t_batch, condition_vec_cg
                )
            ...
            xt = (
                ...
                - sigmas[i + 1] * torch.expm1(h) * eps_theta
            )
            xt = xt * (1.0 - self.fix_mask) + prior * self.fix_mask
