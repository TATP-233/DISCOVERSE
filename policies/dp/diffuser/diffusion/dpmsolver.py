...
def onestep_estimate(xt, pred_theta, i, alphas, sigmas, h, sampler):
    ...
    if sampler[:-2] == "ode_dpm" or sampler == "ddim":
        xt_next = (
            alphas[i] / alphas[i - 1] * xt - sigmas[i] * torch.expm1(h[i]) * pred_theta
        )
    elif sampler[:-2] == "sde_dpm":
        xt_next = (
            alphas[i] / alphas[i - 1] * xt
            - 2.0 * sigmas[i] * torch.expm1(h[i]) * pred_theta
            + sigmas[i] * torch.expm1(2.0 * h[i]).sqrt() * torch.randn_like(xt)
        )
    elif sampler[:-2] == "ode_dpmpp":
        xt_next = (
            sigmas[i] / sigmas[i - 1] * xt - alphas[i] * torch.expm1(-h[i]) * pred_theta
        )
    elif sampler[:-2] == "sde_dpmpp":
        xt_next = (
            sigmas[i] / sigmas[i - 1] * (-h[i]).exp() * xt
            - alphas[i] * torch.expm1(-2.0 * h[i]) * pred_theta
            + sigmas[i]
            * (-1.0 * torch.expm1(-2.0 * h[i])).sqrt()
            * torch.randn_like(xt)
        )
    else:
...
class DPMSolver(DiffusionModel):
    ...
    def predict_function(
        ...
        if predict_noise:
            if self.clip_pred:
                upper_bound = (
                    (x - alpha * self.x_min) / sigma if self.x_min is not None else None
                )
                lower_bound = (
                    (x - alpha * self.x_max) / sigma if self.x_max is not None else None
                )
    ...
    def sample(
        ...
        **kwargs,
        ...
        assert (
            sampler in self.supported_samplers
        ), f"Sampler '{sampler}' is not supported."

        # initialize logger
        log = {
            "sample_history": np.empty((n_samples, sample_steps + 1, *prior.shape))
            if preserve_history
            else None,
        }
        ...
        t = (
            (
                (sample_steps - idx) / sample_steps * self.t_range[1] ** (1 / kappa)
                + idx / sample_steps * self.t_range[0] ** (1 / kappa)
            )
            ** kappa
        ).to(self.device)
        ...
        for i in range(1, sample_steps + 1):
            ...
            if SAMPLER_CONFIG[sampler]["order"] == 2:
                buffer.append(pred_theta.clone())
            ...
            xt = xt * (1.0 - self.fix_mask) + prior * self.fix_mask
            if preserve_history:
                log["sample_history"][:, 1] = xt.cpu().numpy()
        ...
        if (
            ...
            with torch.no_grad():
                logp = self.classifier.logp(
                    xt, t[-1].repeat(n_samples), condition_vec_cg
                )
            log["log_p"] = logp
    ...
    def sample_x(
        ...
        **kwargs,
        ...
        for i in range(1, sample_steps + 1):
            ...
            xt = xt * (1.0 - self.fix_mask) + prior * self.fix_mask
            if preserve_history:
                log["sample_history"][:, 1] = xt.cpu().numpy()  # TODO: BUG logging
        ...
        if SAMPLER_CONFIG[sampler]["order"] == 1 and extra_sample_steps > 0:
            for i in range(extra_sample_steps):
                ...
                pred_theta, log = self.predict_function(
                    xt,
                    t[sample_steps - 1].repeat(n_samples),
                    alphas[sample_steps - 1],
                    sigmas[sample_steps - 1],
                    use_ema=use_ema,
                    requires_grad=requires_grad,
                    predict_noise=SAMPLER_CONFIG[sampler]["predict_noise"],
                    condition_vec_cfg=condition_vec_cfg,
                ...
                xt = onestep_estimate(
                    xt, pred_theta, sample_steps, alphas, sigmas, h, sampler
                )
