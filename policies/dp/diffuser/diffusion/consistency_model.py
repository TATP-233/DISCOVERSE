...
from diffuser.diffusion import DiffusionModel
...
class ContinuousConsistencyModel(DiffusionModel):
    ...
    def __init__(
        ...
        self.pseudo_huber_constant = (
            0.01 if data_dim is None else 0.00054 * np.sqrt(data_dim)
        )

        self.rho = rho
        self.sigma_data, self.sigma_max, self.sigma_min = (
            sigma_data,
            sigma_max,
            sigma_min,
        )
    ...
    def c_skip(self, sigma):
        return self.sigma_data**2 / (
            self.sigma_data**2 + (sigma - self.sigma_min) ** 2
        )
    ...
    def distillation_loss(self, x0, condition=None):
        ...
        assert (
            self.edm is not None
        ), "Please call `prepare_distillation` before distillation."
        ...
        with torch.no_grad():
            condition_vec_cfg = (
                self.edm.model_ema["condition"](condition)
        ...
        with torch.no_grad():
            condition_vec_ema = (
                self.model_ema["condition"](condition)
                if condition is not None
                else None
            )
    ...
    def training_loss(self, x0, condition=None):
        ...
        idx = np.random.choice(
            self.cur_logger.Nk, size=x0.shape[0], p=self.cur_logger.p_sigmas
        )
    ...
    def update(
        self, x0, condition=None, update_ema=True, loss_type="training", **kwargs
    ):
    ...
    def sample(
        ...
        **kwargs,
        ...
        assert (
            w_cg == 0.0 and condition_cg is None
        ), "Consistency Distillation does not support classifier guidance."
