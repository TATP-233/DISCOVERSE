...
class EDMArchetecture(DiffusionModel):
    ...
    def loss(self, x0, condition=None):
        ...
        eps = torch.randn_like(x0) * sigma * (1.0 - self.fix_mask)
        condition = (
            self.model["condition"](condition) if condition is not None else None
        )
        loss = (
            self.loss_weighting(sigma) * (self.D(x0 + eps, sigma, condition) - x0) ** 2
        )
    ...
    def update_classifier(self, x0, condition):
        ...
        eps = torch.randn_like(x0) * sigma * (1.0 - self.fix_mask)
        log = self.classifier.update(
            x0 + eps, at_least_ndim(noise.squeeze(), 1), condition
        )
        return log
    ...
    def dot_x(
        ...
        noise = self.c_noise(sigma)
        unscale = 1.0 / self.scale_s[i] * (1.0 - self.fix_mask) + self.fix_mask
        # ----------------- CFG ----------------- #
        with torch.no_grad():
        ...
        if self.classifier is not None and w_cg != 0.0 and condition_vec_cg is not None:
            log_p, grad = self.classifier.gradients(
                x * unscale, at_least_ndim(noise.squeeze(), 1), condition_vec_cg
            )
    ...
    def sample(
        ...
        **kwargs
        ...
        if prior is None:
            ...
            xt = (
                torch.randn_like(prior, device=self.device)
                * self.sigma_s[0]
                * self.scale_s[0]
            )
        ...
        for i in range(N):
            ...
            delta_x, log = self.dot_x(
                xt, i, use_ema, condition_vec_cfg, w_cfg, condition_cg, w_cg
            )
            delta_t = self.t_s[i] - self.t_s[i + 1]
            x_tp1 = xt - delta_x * delta_t
            if prior is not None:
                x_tp1 = x_tp1 * (1.0 - self.fix_mask) + prior * self.fix_mask
            if solver == "heun":
                if i != N - 1 and self.sigma_s[i + 1] > 0.005:
                    delta_x_2, log = self.dot_x(
                        x_tp1,
                        i + 1,
                        use_ema,
                        condition_vec_cfg,
                    ...
                    x_tp1 = xt - (delta_x + delta_x_2) / 2.0 * delta_t
                    if prior is not None:
                        x_tp1 = x_tp1 * (1.0 - self.fix_mask) + prior * self.fix_mask
        ...
        if (
            ...
            with torch.no_grad():
                logp = self.classifier.logp(
                    xt,
                    at_least_ndim(self.c_noise(self.sigma_s[-1]).squeeze(), 1),
                    condition_cg,
                )
            log["log_p"] = logp
    ...
    def sample_x(
        ...
        if extra_sample_steps > 0:
            ...
            for _ in range(extra_sample_steps):
                ...
                delta_x, log = self.dot_x(
                    xt, N - 1, use_ema, condition_vec_cfg, w_cfg, condition_cg, w_cg
                )
                x_tp1 = xt - delta_x * delta_t
                if prior is not None:
                    x_tp1 = x_tp1 * (1.0 - self.fix_mask) + prior * self.fix_mask
...
class EDM(EDMArchetecture):
    ...
    def set_sample_steps(self, N: int):
        ...
        self.x_weight_s = (
            self.dot_sigma_s / self.sigma_s + self.dot_scale_s / self.scale_s
        )
