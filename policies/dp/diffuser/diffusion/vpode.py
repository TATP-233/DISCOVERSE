...
class VPODE(EDMArchetecture):
    ...
    def set_sample_steps(self, N: int):
        ...
        self.sigma_s = (
            (0.5 * self.beta_d * (self.t_s**2) + self.beta_min * self.t_s).exp() - 1.0
        ).sqrt()
        self.scale_s = 1 / (1 + self.sigma_s**2).sqrt()
        self.dot_sigma_s = (
            0.5
            * (self.sigma_s**2 + 1)
            * (self.beta_d * self.t_s + self.beta_min)
            / self.sigma_s
        )
        self.dot_scale_s = (
            -self.sigma_s / (1 + self.sigma_s**2) ** 1.5 * self.dot_sigma_s
        )
        self.x_weight_s = (
    ...
    def c_noise(self, sigma):
        ...
        t = (
            (self.beta_min**2 - 4 * self.beta_d * scale.log()).sqrt() - self.beta_min
        ) / self.beta_d
