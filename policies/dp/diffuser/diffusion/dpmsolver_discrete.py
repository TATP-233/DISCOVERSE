...
class DPMSolverDiscrete(DiffusionModel):
    def __init__(
        ...
        ] = None,  # be in the shape of `x_shape`
        # Add loss weight
        loss_weight: Union[
            list, np.ndarray, torch.Tensor
        ] = None,  # be in the shape of `x_shape`
        ...
        if self.noise_schedule == "linear":
            beta0, beta1 = 0.1, 20
            self.t_cont = torch.linspace(t_eps, 1.0, diffusion_steps, device=device)
            self.alphas = (
                -(beta1 - beta0) / 4 * (self.t_cont**2) - beta0 / 2 * self.t_cont
            ).exp()
        elif self.noise_schedule == "cosine":
            s = 0.008
            self.t_cont = torch.linspace(t_eps, 0.9946, diffusion_steps, device=device)
            self.alphas = (
                (torch.cos(np.pi / 2 * (self.t_cont + s) / (1 + s))).log()
                - np.log(np.cos(np.pi / 2 * s / (1 + s)))
            ).exp()
        else:
            raise ValueError(
                f"noise_schedule should be 'linear' or 'cosine', but got {self.noise_schedule}."
            )
    ...
    def add_noise(self, x0, idx=None, eps=None):
        if idx is None:
            idx = torch.randint(
                self.diffusion_steps, (x0.shape[0],), device=self.device
            )
    ...
    def loss(self, x0, condition=None):
        ...
        condition = (
            self.model["condition"](condition) if condition is not None else None
        )
        ...
        return (loss * self.loss_weight * (1.0 - self.fix_mask)).mean()
    ...
    def predict_eps_theta(
        ...
        with torch.no_grad():
            if w_cfg != 0.0 and w_cfg != 1.0:
                ...
                condition_vec_cfg = torch.cat(
                    [condition_vec_cfg, torch.zeros_like(condition_vec_cfg)], 0
                )
                pred = model["diffusion"](
                    x.repeat(*repeat_dim), idx.repeat(2), condition_vec_cfg
                )
                pred = w_cfg * pred[:b] + (1.0 - w_cfg) * pred[b:]
            else:
    ...
    def sample_with_ode_dpm_1(
        ...
        t_cond = (
            (
                (sample_steps - idx) / sample_steps * 1.0 ** (1 / kappa)
                + idx / sample_steps * self.t_eps ** (1 / kappa)
            )
            ** kappa
        ).to(self.device)
        t_sample = (
            torch.max(t_cond - 1 / self.diffusion_steps, torch.zeros_like(t_cond))
        ...
        if prior is None:
            xt = (
                torch.randn((n_samples, *self.default_x_shape), device=self.device)
                * temperature
            )
        else:
        ...
        for i in range(1, sample_steps + 1):
            ...
            xt = (
                ...
                - sigmas[i] * torch.expm1(h[i]) * eps_theta
            )
            xt = xt * (1.0 - self.fix_mask) + prior * self.fix_mask
        ...
        if (
            ...
            and condition_cg is not None
        ):
            with torch.no_grad():
                logp = self.classifier.logp(
                    xt, t_sample[-1].repeat(n_samples), condition_vec_cg
                )
            log["log_p"] = logp
    ...
    def sample_with_sde_dpm_1(
        ...
        for i in range(1, sample_steps + 1):
            ...
            xt = (
                alphas[i] / alphas[i - 1] * xt
                - 2.0 * sigmas[i] * torch.expm1(h[i]) * eps_theta
                + sigmas[i] * torch.expm1(2.0 * h[i]).sqrt() * torch.randn_like(xt)
            )
            xt = xt * (1.0 - self.fix_mask) + prior * self.fix_mask
    ...
    def sample_with_sde_dpmpp_1(
        ...
        for i in range(1, sample_steps + 1):
            ...
            xt = (
                ...
                + alphas[i] * (1.0 - (-2.0 * h[i]).exp()) * x_theta
                + sigmas[i] * (1.0 - (-2.0 * h[i]).exp()).sqrt() * torch.randn_like(xt)
            )
            xt = xt * (1.0 - self.fix_mask) + prior * self.fix_mask
    ...
    def sample_with_ode_dpmpp_2(
        ...
        x_theta_t0 = x_theta_t0 * (1.0 - self.fix_mask) + prior * self.fix_mask
        ...
        x_theta_t1 = x_theta_t1 * (1.0 - self.fix_mask) + prior * self.fix_mask
        ...
        for i in range(2, sample_steps + 1):
            ...
            if i < sample_steps + 1:
                ...
                x_theta_ti = x_theta_ti * (1.0 - self.fix_mask) + prior * self.fix_mask
