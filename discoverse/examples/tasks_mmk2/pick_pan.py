...
cfg.mjcf_file_path = "mjcf/tasks_mmk2/pick_pan.xml"
        ...
        try:
            if stm.trigger():
                if stm.state_idx == 0:  # 降高度
                    ...
                    euler_angles = Rotation.from_matrix(tmat_pan[:3, :3]).as_euler(
                        "zyx", degrees=False
                    )
                    sim_node.rgt_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(
                        target_posi
                    )
                    ...
                    target_posi = tmat_box[:3, 3] + np.array(
                        [-0.35, 0.0, 0.25]
                    )  # y,x,z
                    # 将旋转矩阵转换为欧拉角 (指定旋转轴顺序为 'zyx')
                    euler_angles = Rotation.from_matrix(tmat_box[:3, :3]).as_euler(
                        "zyx", degrees=False
                    )
                    sim_node.rgt_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(
                        target_posi
                    ...
                    sim_node.setArmEndTarget(
                        ...
                        sim_node.sensor_rgt_arm_qpos,
                        Rotation.from_euler(
                            "zyx", [np.pi / 2.1, 0, np.pi / 2 + np.pi / 3]
                        ).as_matrix(),
                    )
                    sim_node.tctr_rgt_gripper[:] = 0
