...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the task, e.g. plate_coffeecup",
    )
    parser.add_argument(
        "--scheme_json",
        type=str,
        required=True,
        help="json-file name of the strategy, e.g. plate_coffeecup",
    )
    parser.add_argument(
        "--max_time_s", type=int, default=30, help="run task max time in seconds"
    )
    ...
    parser.add_argument(
        "--vis",
        action="store_true",
        help="visualize, if --auto is set, this will be ignored",
    )
    parser.add_argument(
        "--dim17", action="store_true", help="genegrate 17 joint num mmk2 data"
    )
    ...
    try:
        task_module = importlib.import_module(
            f"discoverse.examples.tasks_mmk2.{args.task_name}"
        )
            ...
            if stm.trigger():
                if stm.state_idx < stm.max_state_cnt:
                    ...
                    if left_arm["movement"] == "move":
                        ...
                        left_arm_rot = Rotation.from_euler(
                            "zyx", left_arm["rotation_robot_local"]
                        ).as_matrix()
                        ...
                        sim_node.lft_arm_target_pose[
                            :
                        ] = sim_node.get_tmat_wrt_mmk2base(target_posi1)
                    ...
                    if right_arm["movement"] == "move":
                        ...
                        right_arm_rot = Rotation.from_euler(
                            "zyx", right_arm["rotation_robot_local"]
                        ).as_matrix()
                        ...
                        sim_node.rgt_arm_target_pose[
                            :
                        ] = sim_node.get_tmat_wrt_mmk2base(target_posi2)
                        sim_node.setArmEndTarget(
                            sim_node.rgt_arm_target_pose,
                            sim_node.arm_action,
                            "r",
