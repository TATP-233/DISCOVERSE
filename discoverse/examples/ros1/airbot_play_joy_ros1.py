...
class AirbotPlayJoyCtl(AirbotPlayBase):
    def __init__(self, config: AirbotPlayCfg):
        ...
        self.tar_end_pose = np.array([0.295, -0.0, 0.219])
    ...
    def teleopProcess(self):
        ...
        if (
            self.teleop.joy_cmd.axes[0]
            or self.teleop.joy_cmd.axes[1]
            or self.teleop.joy_cmd.axes[4]
        ):
            calc_ik = True
        ...
        if (
            self.teleop.joy_cmd.axes[3]
            or self.teleop.joy_cmd.axes[6]
            or self.teleop.joy_cmd.axes[7]
        ):
            calc_ik = True
        ...
        if calc_ik:
            ...
            try:
                tarjq = self.arm_fik.properIK(
                    self.tar_end_pose, rot, self.sensor_joint_qpos[:6]
                )
            ...
            if not tarjq is None:
                ...
                rospy.logwarn(
                    "Fail to solve inverse kinematics trans={} euler={}".format(
                        self.tar_end_pose, self.tar_end_euler
                    )
                )
        ...
        if self.teleop.joy_cmd.axes[2] - self.teleop.joy_cmd.axes[5]:
            ...
            self.tar_jq[6] = np.clip(self.tar_jq[6], 0, 1.0)
    ...
    def printMessage(self):
        ...
        print(
            "target end posi  = {}".format(
                np.array2string(self.tar_end_pose, separator=", ")
            )
        )
        print(
            "target end euler = {}".format(
                np.array2string(self.tar_end_euler, separator=", ")
            )
        )
