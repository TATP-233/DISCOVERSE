...
class RobotArmSync(MMK2Base):
    ...
    def __init__(self, config: MMK2Cfg, robot_name: str):
        ...
        self.lft_arm_suber = rospy.Subscriber(
            f"/{robot_name}/arms/left/joint_states", JointState, self.lft_arm_callback
        )
        self.rgt_arm_suber = rospy.Subscriber(
            f"/{robot_name}/arms/left/joint_states", JointState, self.rgt_arm_callback
        )
    ...
    def lft_arm_callback(self, msg: JointState):
        if len(msg.position) == 7:
            ...
            rospy.logwarn(
                f"Invalid <LEFT> arm joint state message, please check. msg.position = {np.array(msg.position)}"
            )
    ...
    def rgt_arm_callback(self, msg: JointState):
        if len(msg.position) == 7:
            ...
            rospy.logwarn(
                f"Invalid <RIGHT> arm joint state message, please check. msg.position = {np.array(msg.position)}"
            )
