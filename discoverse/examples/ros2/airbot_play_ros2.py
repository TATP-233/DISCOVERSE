...
class AirbotPlayROS2(AirbotPlayBase, Node):
    def __init__(self, config: AirbotPlayCfg):
        ...
        self.joint_state_puber = self.create_publisher(
            JointState, "/airbot_play/joint_states", 5
        )
        self.joint_state = JointState()
        self.joint_state.name = [f"joint{i+1}" for i in range(6)] + ["gripper"]

        self.bridge = CvBridge()
        self.side_color_puber = self.create_publisher(
            Image, "/airbot_play/side_camera/color/image_raw", 2
        )
        self.side_depth_puber = self.create_publisher(
            Image, "/airbot_play/side_camera/aligned_depth_to_color/image_raw", 2
        )
        self.arm_color_puber = self.create_publisher(
            Image, "/airbot_play/arm_camera/color/image_raw", 2
        )
        self.arm_depth_puber = self.create_publisher(
            Image, "/airbot_play/arm_camera/aligned_depth_to_color/image_raw", 2
        )

        self.arm_cmd_suber = self.create_subscription(
            Float64MultiArray,
            "/airbot_play/forward_position_controller/commands",
            self.cmd_arm_callback,
            5,
        )
    ...
    def thread_pubros2topic(self, freq=30):
        ...
        while rclpy.ok() and self.running:
            ...
            side_color_img_msg = self.bridge.cv2_to_imgmsg(
            ...
            side_depth_img = np.array(
                np.clip(self.obs["depth"][0] * 1e3, 0, 65535), dtype=np.uint16
            )
            side_depth_img_msg = self.bridge.cv2_to_imgmsg(
                side_depth_img, encoding="mono16"
            )
            ...
            arm_color_img_msg = self.bridge.cv2_to_imgmsg(
            ...
            arm_depth_img = np.array(
                np.clip(self.obs["depth"][1] * 1e3, 0, 65535), dtype=np.uint16
            )
            arm_depth_img_msg = self.bridge.cv2_to_imgmsg(
                arm_depth_img, encoding="mono16"
            )
    ...
    def printMessage(self):
        ...
        print(
            "joint q     = {}".format(
                np.array2string(self.sensor_joint_qpos, separator=", ")
            )
        )
        print(
            "joint v     = {}".format(
                np.array2string(self.sensor_joint_qvel, separator=", ")
            )
        )

        print(
            "sensor end posi  = {}".format(
                np.array2string(self.sensor_endpoint_posi_local, separator=", ")
            )
        )
        print(
            "sensor end euler = {}".format(
                np.array2string(
                    Rotation.from_quat(
                        self.sensor_endpoint_quat_local[[1, 2, 3, 0]]
                    ).as_euler("xyz"),
                    separator=", ",
                )
            )
