...
class SimNode(AirbotPlayTaskBase):
    def domain_randomization(self):
        # 杯子
        self.mj_data.qpos[self.nj + 1 + 0] += 2.0 * (np.random.random() - 0.5) * 0.1
        self.mj_data.qpos[self.nj + 1 + 1] += 2.0 * (np.random.random() - 0.5) * 0.03

        wood_base_x_bias = 2.0 * (np.random.random() - 0.5) * 0.05
        # 盘子
        self.mj_data.qpos[self.nj + 7 * 1 + 1 + 0] += (
            2.0 * (np.random.random() - 0.5) * 0.05 + wood_base_x_bias
        )
        self.mj_data.qpos[self.nj + 7 * 1 + 1 + 1] += (
            2.0 * (np.random.random() - 0.5) * 0.02
        )
        ...
        # 杯盖
        self.mj_data.qpos[self.nj + 7 * 3 + 1 + 0] += (
            2.0 * (np.random.random() - 0.5) * 0.05
        )
        self.mj_data.qpos[self.nj + 7 * 3 + 1 + 1] += (
            2.0 * (np.random.random() - 0.5) * 0.05
        )
    ...
    def check_success(self):
        ...
        return (
            (abs(tmat_cup[2, 2]) > 0.99)
            and np.hypot(
                tmat_plate[0, 3] - tmat_cup[0, 3], tmat_plate[1, 3] - tmat_cup[1, 3]
            )
            < 0.02
            and np.hypot(
                tmat_lid[0, 3] - tmat_cup[0, 3], tmat_lid[1, 3] - tmat_cup[1, 3]
            )
            < 0.02
        )
...
if __name__ == "__main__":
    ...
    trmat_cup = Rotation.from_euler(
        "xyz", [0, np.pi * 0.4, 0], degrees=False
    ).as_matrix()
    trmat_lid = Rotation.from_euler(
        "xyz", [0, np.pi * 0.5, 0], degrees=False
    ).as_matrix()
            ...
            if stm.trigger():
                if stm.state_idx == 0:  # 伸到杯子前
                    ...
                    sim_node.target_control[:6] = arm_fik.properIK(
                        tmat_tgt_local[:3, 3], trmat_cup, sim_node.mj_data.qpos[:6]
                    )
                    ...
                    tmat_coffee[:3, 3] = (
                        tmat_coffee[:3, 3]
                        + 0.06 * tmat_coffee[:3, 1]
                        + 0.05 * tmat_coffee[:3, 2]
                    )
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_coffee
                    sim_node.target_control[:6] = arm_fik.properIK(
                        tmat_tgt_local[:3, 3], trmat_cup, sim_node.mj_data.qpos[:6]
                    ...
                    sim_node.target_control[:6] = arm_fik.properIK(
                        tmat_tgt_local[:3, 3], trmat_lid, sim_node.mj_data.qpos[:6]
                    )
