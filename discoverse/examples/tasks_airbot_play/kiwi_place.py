...
class SimNode(AirbotPlayTaskBase):
    def domain_randomization(self):
        ...
        random_bias_xy[0] = 2.0 * (np.random.random() - 0.5) * 0.08
        random_bias_xy[1] = 2.0 * (np.random.random() - 0.5) * 0.03
        ...
        # 随机 碗位置
        self.mj_data.qpos[self.nj + 2 * 7 + 1 + 0] += (
            2.0 * (np.random.random() - 0.5) * 0.05
        )
        self.mj_data.qpos[self.nj + 2 * 7 + 1 + 1] += (
            2.0 * (np.random.random() - 0.5) * 0.03
        )
    ...
    def check_success(self):
        ...
        return (abs(tmat_bowl[2, 2]) > 0.99) and np.hypot(
            tmat_jujube[0, 3] - tmat_bowl[0, 3], tmat_jujube[1, 3] - tmat_bowl[1, 3]
        ) < 0.02
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_segment", action="store_true", help="save segment videos"
    )
    ...
    if args.save_segment:
        cfg.obs_depth_cam_id = list(
            set(
                cfg.obs_rgb_cam_id
                + ([] if cfg.obs_depth_cam_id is None else cfg.obs_depth_cam_id)
            )
        )
        ...
        samples = SampleforDR(
            objs=cfg.obj_list[2:],
            robot_parts=cfg.rb_link_list,
            cam_ids=cfg.obs_rgb_cam_id,
            save_dir=os.path.join(save_dir, "segment"),
    ...
    trmat = Rotation.from_euler("xyz", [0.0, 1.4, 0.0], degrees=False).as_matrix()
