...
class SkyRoverOnCarCfg(BaseConfig):
    ...
    sync = True
    headless = False
    render_set = {"fps": 60, "width": 1920, "height": 1080}  # 30,
        ...
        if action == glfw.PRESS:
            if key == glfw.KEY_S:
                for camid, img in self.obs["img"].items():
                    ...
                    cv2.imwrite(
                        os.path.join(DISCOVERSE_ROOT_DIR, f"img_{camid}.png"),
...
if __name__ == "__main__":
    ...
    viewer = mujoco.viewer.launch_passive(exec_node.mj_model, exec_node.mj_data)
    ...
    while exec_node.running:
        ...
        """ Mission execution """
        cc.target_pos = [0.0, 0.0, 0.0]  # Dummy (for logging)
        ...
        exec_node.mj_data.qpos[64] = 3 * np.sin(
            0.1 * (time.time() - sine_motion_start_time)
        )
        ...
        exec_node.mj_data.qpos[82] = -2 * np.sin(
            0.1 * (time.time() - sine_motion_start_time)
        )
        ...
        (
            ...
            skyrover_pqr,
            skyrover_eul,
        ) = cc.multi_robot.get_skyrover_state(verbose=False)
        # rm2_pos, rm2_vel, _, rm2_pqr, rm2_eul = cc.multi_robot.get_rm2_state(verbose=False)
        cc.logger.log_quadrotor_state(
            skyrover_pos,
            skyrover_vel,
            skyrover_quat,
            skyrover_pqr,
    ...
    json_data = {"time": time_data, "qpos": qpos_data}
    ...
    timestamp = now.strftime(
        "discoverse/examples/skyrover_on_rm2car/log/%Y-%m-%d %H:%M:%S"
    )
    ...
    mediapy.write_video(
        f"discoverse/examples/skyrover_on_rm2car/log/%Y-%m-%d %H:%M:%S_third_view.mp4",
        img_lst_third_view,
        fps=30,
    )
    mediapy.write_video(
        f"discoverse/examples/skyrover_on_rm2car/log/%Y-%m-%d %H:%M:%S_front_view.mp4",
        img_lst_first_person_front,
        fps=30,
    )
    mediapy.write_video(
        f"discoverse/examples/skyrover_on_rm2car/log/%Y-%m-%d %H:%M:%S_down_view.mp4",
        img_lst_first_person_down,
        fps=30,
    )
