...
if __name__ == "__main__":
    ...
    cfg.gs_model_dict[
        "background"
    ] = "scene/tsimf_library_0/point_cloud_for_skyrover.ply"
    ...
    while exec_node.running:
        if cnt * exec_node.delta_t < 1.0:
            action[1] += 80 * exec_node.delta_t / 1.0
            action[6] += 80 * exec_node.delta_t / 1.0
        elif cnt * exec_node.delta_t < 1.5:
            action[0] -= 545 * exec_node.delta_t / 0.5
        elif cnt * exec_node.delta_t < 2.0:
            ...
            action[7] = action[8] = 0.0
            action[0] += 545 * exec_node.delta_t / 1.0
        elif cnt * exec_node.delta_t < 4.5:
            action[1] -= 80 * exec_node.delta_t / 1.0
            action[6] -= 80 * exec_node.delta_t / 1.0
        elif cnt * exec_node.delta_t < 5.0:
            action[0] -= 545 * exec_node.delta_t / 0.5
        elif cnt * exec_node.delta_t < 6.0:
            action[[4, 5, 9, 10]] = 10
            exec_node.mj_data.qpos[0] = last_x
            exec_node.mj_data.qpos[2] = last_z + 0.5 * exec_node.delta_t / 1.0
            exec_node.mj_data.qpos[[3, 4, 5, 6]] = [1, 0, 0, 0]
        elif cnt * exec_node.delta_t < 7.0:
            exec_node.mj_data.qpos[0] = last_x
            exec_node.mj_data.qpos[2] = last_z - 0.5 * exec_node.delta_t / 1.0
            exec_node.mj_data.qpos[[3, 4, 5, 6]] = [1, 0, 0, 0]
        else:
