...
class SkyRoverOnCarBase(SimulatorBase):
    def __init__(self, config: BaseConfig):
        ...
        self.robots = {
            name: {"joints": {}, "bodies": {}, "sites": {}} for name in self.robot_names
        }  # robot attributes
        self.actuator_mapping = self._parse_actuators_from_xml(
            self.mjcf_file
        )  # robot attributes
    ...
    def set_joint_position(self, robot_name, joint_name, position):
        ...
        if self._sanity_check(
            robot_name=robot_name, attribute_name=joint_name, type="joints"
        ):
            ...
            print(
                f"[ERROR] Sanity check failed for robot '{robot_name}' and attribute '{joint_name}'. Check that the attribute exists and the robot is initialized properly."
            )
    ...
    def set_site_control_value(self, robot_name, site_name, control_value):
        ...
        if not site_name.startswith(robot_name):
            print(
                f"[ERROR] Site '{site_name}' does not start with the robot name prefix '{robot_name}'."
            )
            return
    ...
    def get_root_body_pose(self, robot_name, root_body_name):
        ...
        if robot_name != root_body_name:
            print(
                f"[ALERT] The root body name is incorrectly specified. Expected: '{robot_name}', but got: '{root_body_name}'. Please verify the input or robot XML file. Make sure that root body has the same name as the robot name."
            )
        ...
        joint_id = self._get_joint_id_from_body_id(
            body_id=body_id, qpos_qvel_flag="qpos"
        )
        ...
        if joint_id is not None:
            ...
            if DEBUGGING_LINE:
                print(
                    f"Body: {root_body_name} | ID: {body_id} | Position (qpos): {pos}"
                )  # Debugging line
            return pos, quat

        print(
            f"[ERROR] No specific joint found for body '{root_body_name}' or joint type not handled."
        )
        return None
    ...
    def get_root_body_velocity(self, robot_name, root_body_name):
        ...
        for root_body_name, body_id in self.robots[robot_name]["bodies"].items():
            # Find the joint associated with this body
            joint_id = self._get_joint_id_from_body_id(
                body_id=body_id, qpos_qvel_flag="qvel"
            )
            ...
            if joint_id is not None:
                ...
                if DEBUGGING_LINE:
                    print(
                        f"Body: {root_body_name} | ID: {body_id} | Velocity (qvel): {vel}"
                    )  # Debugging line
                return vel, pqr

        print(
            f"[ERROR] No specific free joint found for body '{root_body_name}' to get linear velocity."
        )
        return None
    ...
    def _initialize_robot(self, robot_name):
        ...
        if (
            not self.robots[robot_name]["bodies"]
            or not self.robots[robot_name]["joints"]
        ):
            print(
                "[ERROR] To properly manage joints and other properties for multiple robots using this script,",
    ...
    def _sanity_check(self, robot_name, attribute_name, type):
        ...
        if type not in ["joints", "sites"]:
            ...
            if attribute_name not in self.robots[robot_name][type]:
                print(
                    f"[ERROR] Site {attribute_name} not found for robot {robot_name}."
                )
                return False
            else:
                return True
    ...
    def _get_joint_id_from_body_id(self, body_id, qpos_qvel_flag):
        ...
        for j_id, b_id in enumerate(
            self.mj_model.jnt_bodyid
        ):  # jnt_bodyid: ID of the body's joint
            if b_id == body_id:
                return joint_addr[
                    j_id
                ]  # Return the start index for the joint in `qpos` or `qvel` data structure
    ...
    def switch_locomotion(self, locomode="drone", switching_duration=5):
        ...
        if locomode == "drone":
            ...
            folder_joint_increment, self.switching_time = self._incremental_change(
                79.8, 0, switching_duration, "decrease", self.switching_time
            )
            stretch_joint_increment, self.switching_time = self._incremental_change(
                -545, 0, switching_duration, "increase", self.switching_time
            )

            if (
                self.get_joint_position("skyrover", "skyrover_stretch_joint")
                < stretch_joint_drone
            ):
                self.set_joint_position(
                    "skyrover", "skyrover_stretch_joint", stretch_joint_increment
                )
            if (
                self.get_joint_position("skyrover", "skyrover_folder1_joint")
                > folder1_joint_drone
            ):
                self.set_joint_position(
                    "skyrover", "skyrover_folder1_joint", folder_joint_increment
                )
            if (
                self.get_joint_position("skyrover", "skyrover_folder2_joint")
                > folder2_joint_drone
            ):
                self.set_joint_position(
                    "skyrover", "skyrover_folder2_joint", folder_joint_increment
                )
            if (
                self.get_joint_position("skyrover", "skyrover_stretch_joint")
                >= stretch_joint_drone
                and self.get_joint_position("skyrover", "skyrover_folder1_joint")
                <= folder1_joint_drone
                and self.get_joint_position("skyrover", "skyrover_folder2_joint")
                <= folder2_joint_drone
            ):
                ready_flag = True
                self.locomode = "drone"
            ...
            # Compute joint position increments
            folder_joint_increment, self.switching_time = self._incremental_change(
                0, 79.8, switching_duration, "increase", self.switching_time
            )
            stretch_joint_increment, self.switching_time = self._incremental_change(
                0, -545, switching_duration, "decrease", self.switching_time
            )
            ...
            raise TypeError(
                "[ERROR] Non-existing locomotion mode. ['drone', 'rover'] are the only available options for now."
            )
    ...
    def get_skyrover_state(self, verbose=False, robot="skyrover"):
        ...
        """
        pos, quat = self.get_root_body_pose(
            "skyrover", robot
        )  # robot position (x, y, z), robot orientation (w, x, y, z)
        vel, pqr = self.get_root_body_velocity(
            "skyrover", robot
        )  # robot linear velocity (x, y, z), body-fixed angular rate (p, q, r)

        # Convert quaternion to Euler angles
        from scipy.spatial.transform import Rotation as R

        rotation = R.from_quat(quat[[1, 2, 3, 0]])  # convert to (x, y, z, w) format
        eul = rotation.as_euler("ZYX", degrees=False)[
            ::-1
        ...
        if verbose:
            ...
            print(
                f"  Euler Angle (roll, pitch, yaw): {roll:.2f}°, {pitch:.2f}°, {yaw:.2f}°"
            )
    ...
    def skyrover_drive(self, diff_drive_ctrl):
        ...
        if self.locomode != "rover":
            raise ValueError(
                "[ERROR] Driving command can be executed only when SkyRover is in rover mode."
            )
        else:
            self.set_joint_position(
                "skyrover", "skyrover_rotor1_joint", diff_drive_ctrl[0]
            )
            self.set_joint_position(
                "skyrover", "skyrover_rotor2_joint", diff_drive_ctrl[1]
            )
            self.set_joint_position(
                "skyrover", "skyrover_rotor3_joint", diff_drive_ctrl[2]
            )
            self.set_joint_position(
                "skyrover", "skyrover_rotor4_joint", diff_drive_ctrl[3]
            )
    ...
    def skyrover_fly(self, rotor_thrust, robot):
        ...
        if self.locomode != "drone":
            raise ValueError(
                "[ERROR] Flying command can be executed only when SkyRover is in drone mode."
            )
        else:
    ...
    def _incremental_change(
        ...
        if change_type == "increase" and start_value >= target_value:
            raise ValueError(
                "start_value must be less than target_value for an increase."
            )
        if change_type == "decrease" and start_value <= target_value:
            raise ValueError(
                "start_value must be greater than target_value for a decrease."
            )
        ...
        if change_type == "increase":
            ...
            current_value = min(
                start_value + change, target_value
            )  # Ensure it does not exceed target_value
        elif change_type == "decrease":
            change = (elapsed_time / duration) * (start_value - target_value)
            current_value = max(
                start_value - change, target_value
            )  # Ensure it does not go below target_value
