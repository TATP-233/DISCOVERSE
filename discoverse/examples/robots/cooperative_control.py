...
class CooperativeAerialGroundControl:
    def __init__(self, multi_robot):
        ...
        self.target_pos = [0.0, 0.0, 0.0]  # dummy (for logging)
    ...
    def do_mission(self, mission_phase):
        ...
        if mission_phase == 0:
            ...
            if time.time() - self.mission_start_time > 5:
                ...
                print(
                    f"Simulation started. Executing mission phase #{self.mission_phase}..."
                )
        ...
        if mission_phase == 3:
            ...
            if self.mission_completed == False:
                ...
                pos, _, _, _, _ = self.multi_robot.get_skyrover_state()
                self.waypoints["phase_3"] = [[pos[0], pos[1], alt_target]]
            else:
                """Get states"""
                ...
                if len(self.waypoints["phase_3"]) == 1:
                    self.waypoints["phase_3"].append(
                        [temp_x_1, 0.8 * temp_y_1, alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_1, 0.6 * temp_y_1, alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_1, 0.4 * temp_y_1, alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_1, 0.2 * temp_y_1, alt_target]
                    )
                    self.waypoints["phase_3"].append([temp_x_2, temp_y_2, alt_target])
                    for i in range(100):
                        self.waypoints["phase_3"].append(
                            [(1 - i / 100) * temp_x_3, temp_y_3, alt_target]
                        )
                    self.waypoints["phase_3"].append([temp_x_4, temp_y_4, alt_target])
                    for i in range(100):
                        self.waypoints["phase_3"].append(
                            [i / 100 * temp_x_5, temp_y_5, alt_target]
                        )
                    self.waypoints["phase_3"].append([temp_x_5, temp_y_5, alt_target])
                    self.waypoints["phase_3"].append([temp_x_6, temp_y_6, alt_target])
                    self.waypoints["phase_3"].append(
                        [temp_x_6, temp_y_6, 0.8 * alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_6, temp_y_6, 0.6 * alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_6, temp_y_6, 0.4 * alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_6, temp_y_6, 0.2 * alt_target]
                    )
                    self.waypoints["phase_3"].append(
                        [temp_x_6, temp_y_6, 0.1 * alt_target]
                    )
                ...
                self.multi_robot.mj_data.ctrl[9] = dummy_rpm
                self.multi_robot.mj_data.ctrl[10] = dummy_rpm
                self.flight_ctrl.D_COEFF_TOR = np.array([20000.0, 20000.0, -2880.0])
                wp_completed, target_pos = self.waypoint_checker(
                    pos=skyrover_pos, phase="phase_3"
                )
                ...
                if not wp_completed:
                    ...
                    self.mission_start_time = (
                        time.time()
                    )  # initialization for next mission
    ...
    def execute_rm2(self, target_pos):
        ...
        control_input = compute_diff_drive_control(
            pid_linear=self.diffd_lin_ctrl,
    ...
    def execute_skyrover_fly(
        self,
        target_pos=[0.0, 0.0, 0.0],
        target_eul=[0.0, 0.0, 0.0],
        target_vel=[0.0, 0.0, 0.0],
        target_pqr=[0.0, 0.0, 0.0],
        land_flag=False,
        robot="skyrover",
    ):
        pos, vel, quat, pqr, eul = self.multi_robot.get_skyrover_state(robot=robot)
        state = np.concatenate((pos, quat, vel, pqr))
        rpm, err_pos, err_rpy = self.flight_ctrl.compute_control_from_state(
    ...
    def execute_skyrover_fly_background(
        self,
        target_pos=[0.0, 0.0, 0.0],
        target_eul=[0.0, 0.0, 0.0],
        target_vel=[0.0, 0.0, 0.0],
        ...
        pos, vel, quat, pqr, eul = self.multi_robot.get_skyrover_state(robot=robot)
        state = np.concatenate((pos, quat, vel, pqr))
        rpm, err_pos, err_rpy = self.flight_ctrl_2.compute_control_from_state(
            control_timestep=self.dt,
            state=state,
            target_pos=target_pos,
            target_rpy=target_eul,
    ...
    def _get_all_robot_state(self):
        rm2_pos, rm2_vel, rm2_quat, rm2_pqr, rm2_eul = self.multi_robot.get_rm2_state(
            verbose=False
        )
        (
            skyrover_pos,
            ...
            skyrover_pqr,
            skyrover_eul,
        ) = self.multi_robot.get_skyrover_state(verbose=False)
        skyrover_state = np.concatenate(
            (skyrover_pos, skyrover_quat, skyrover_vel, skyrover_pqr)
        )
